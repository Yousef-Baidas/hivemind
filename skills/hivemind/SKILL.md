...
7½. **Human review**: milestone complete and `WAVE-GREEN` → read `references/review.md`. Spawn `hive-guide`, print its brief, stop. Relay questions to the guide. `ACCEPT` → next milestone. `CHANGES` → tickets, step 3, same gate again. Nothing dispatches while a review is open.
