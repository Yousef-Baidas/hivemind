...
1. Pending: `gh issue list --label needs-human --state all --json number,title,url -q '.[] | "\(.number) \(.title)"'`. One → open it. Several → list, ask which. Auto-accepted ones (last comment `AUTO-ACCEPT`) count as pending; a human verdict replaces the auto one, say so.
